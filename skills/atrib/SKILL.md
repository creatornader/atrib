...
- **Caller-enabled**: [§6.3](https://github.com/creatornader/atrib/blob/main/atrib-spec.md#63-verifier-consultation-algorithm) identity consultation runs directory lookup, claim parsing, and revocation checks by default. Caller-supplied trust roots and callbacks enable anchor discovery and freshness, directory checkpoint signature verification, append-only audit verification, and AKD lookup-proof validation. Missing inputs remain explicit warnings. Step 3 still counts cosignature lines without cryptographically verifying them against a trusted witness-key set inside `resolveIdentity()`.
...
- **Other open implementation work**: HKDF sub-agent identity derivation, the emergency-key compromise path, and generic verifier-owned archive fetching remain unimplemented. Read primitives and the Explorer can retrieve archive bodies today, while verification APIs still require the caller to supply the body or evidence. Per-operation directory anchoring is implemented; a separate periodic anchoring loop is not.
