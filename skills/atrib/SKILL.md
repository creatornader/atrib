...
1. Read the SessionStart hook output above (if present). The hook ran
   ...
   • High-importance records surfaced? Those are decision-critical context.
...
2. Mentally identify the conversation's likely write-primitive moments:
   ...
   • atrib-annotate: which past records should I mark as decision-critical?
...
- A past observation is decision-critical for the session you're in. Annotate with `importance: high` or `critical` so recall surfaces it ahead of flat scans.
...
4. **What importance signal does future-me need?** If this is one of the decision-critical records of the session, follow the emit with an `atrib-annotate` referencing the new record's hash with `importance: high` or `critical`.

If you can't answer #1 in one line, you don't need to sign yet.

## Code-shape examples

### Pattern 1: decision-critical observation + annotation
...
- "This decision is decision-critical. Signing observation + annotation so future-self finds it under [topic]."
...
1. Will future-me, or another agent, want to find this with full context?
   → Sign explicitly via emit. Add an annotation if it's decision-critical.
...
5. Am I about to make a decision-critical claim externally (commit, PR, briefing)?
