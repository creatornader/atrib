...
- **Role terms in `baton` facts.** `target_harness_role` uses role vocabulary (`successor-session`, `relay-executor`, `loop-layer`); operator-internal names belong in the private packet body, not in signed content or envelope facts.
