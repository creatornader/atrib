...
Passing that proof means the host binary can satisfy the local contract in isolation. Live dogfood adoption still needs a separate topology report. As of the first watcher-WAL adoption slice, the local knowledge-base drain is wired through a coordinator and a live probe reached the public log. As of the first long-lived route adoption slice, the Hermes gateway launch agent carries a shadow-mode local-substrate endpoint for the scheduled heartbeat producer. Startup-spawn process-count work now has two gates: per-thread collapse onto `atrib-primitives`, and agent-scoped loopback HTTP hosting for cross-thread process reuse.

That report now lives at [`scripts/report-local-substrate-topology.mjs`](../../scripts/report-local-substrate-topology.mjs). It reads process rows, sanitized Codex and Claude Code MCP config summaries, launchd service metadata, an optional supervised route registry at `~/.atrib/local-substrate/routes.json`, coordinator health probes, primitive-runtime HTTP health probes, and Agent Bridge wrapper/upstream process groups. It separates seven gates: coordinator health, startup-spawn MCP process collapse, startup-spawn config, agent-scoped primitive HTTP hosting, Agent Bridge wrapper footprint, watcher-WAL routing, and long-lived-agent routing. The process inventory groups standalone primitive children into launch generations and marks them as obsolete config drift when the current startup-spawn config already declares `atrib-primitives` and no longer declares the standalone primitives. The long-lived gate now requires every known long-lived agent route in the report to point at a healthy coordinator endpoint. Known Hermes and OpenClaw launchd labels remain compatibility shortcuts, but future harnesses should self-declare safe `ATRIB_LOCAL_SUBSTRATE_ENDPOINT` and `ATRIB_AGENT` values or add a registry entry with `kind: "long-lived-agent"`. The fixture-backed checker runs in `pnpm doc-sync`, so a healthy isolated host proof cannot hide a mixed live topology with duplicated primitive bundles, duplicated Agent Bridge wrappers, obsolete children held by a still-running app server, missing agent-scoped primitive hosting, partial long-lived agent routing, or absent supervised long-lived producer routing.
