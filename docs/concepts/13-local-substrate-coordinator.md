...
**Builds on**: [Integration patterns](10-integration-patterns.md), [The chain](04-the-chain.md), [The cognitive primitives: two verbs, seven aliases](11-cognitive-primitives.md)
...
`@atrib/primitives-runtime` is a separate process-count slice. Per the attest/recall rename ([D164](../../DECISIONS.md#d164-attestrecall-verb-rename-and-primitive-surface-collapse)), it mounts three primitives in process (attest write home, recall read home, summarize) and exposes the seventeen-tool union (fifteen legacy aliases plus `attest` plus `recall`) through one local runtime named `atrib-primitives`. Direct stdio mode collapses one harness thread from three primitive child processes to one. Streamable HTTP mode lets one startup-spawn agent profile share a loopback host process across that profile's threads, with one mounted primitive backend per host process, one outer MCP session transport per client, and no per-primitive OS children. Stdio-to-HTTP proxy mode is for stdio-only clients such as Claude Code and Claude Desktop: the client still speaks stdio MCP, but the proxy forwards to an agent-scoped Streamable HTTP host and does not mount the primitive backend itself. The package is private to the workspace and meant for dogfood configs where Codex, Claude Code, Claude Desktop, or another startup-spawn harness would otherwise launch three atrib primitive child processes per thread. A host serving multiple agent profiles needs separate primitive runtime processes until the MCP client supplies a per-request profile boundary or a host supervisor exposes an explicit profile-routing contract. It does not own signing policy, WAL commit, receipt join-back, queue health, or cross-harness supervision. Those stay with the local substrate coordinator track.
...
### Global supervisor boundary

One global shared primitive backend is not safe. The current primitive servers bind profile state through process environment and module-level defaults: key resolution, mirror paths, local-substrate endpoint, active-session profile fallback, and some read-side mirror/context defaults. If Codex, Claude Code, and Claude Desktop share that backend without an explicit route, one profile can sign or read through another profile's configuration.

One host supervisor process can be safe if it is not a shared backend. It has to contain isolated profile runtimes and select one through an authenticated profile boundary on each request or session. The topology report recognizes that future shape only when the primitive health report advertises `profile_routing.mode: "explicit"`, `profile_routing.isolated_backends: true`, and per-profile route entries with `agent`, `local_substrate_endpoint`, and context policy. Without that contract, two configs pointing at one primitive endpoint remain a profile mismatch and keep broad readiness closed.

The practical sequence is:

1. Keep the current per-profile HTTP hosts as the live dogfood baseline.
2. Add a supervisor that starts or owns isolated `codex`, `claude-code`, `claude-desktop`, and future profile runtimes inside one OS process.
3. Route each MCP session or request by loopback-only profile token, path segment, or another explicit local-auth mechanism.
4. Move read indexes into shared caches only after signing, mirror append, and context fallback stay profile-scoped.
...
- every active host-owned primitive HTTP profile either exposes a fresh active-session state file or requires explicit `context_id` before write primitives sign; idle primitive HTTP profiles are reported separately
