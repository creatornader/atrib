...
That report now lives at [`scripts/report-local-substrate-topology.mjs`](../../scripts/report-local-substrate-topology.mjs). It reads process rows, sanitized Codex and Claude Code MCP config summaries, launchd service metadata, coordinator health probes, and primitive-runtime HTTP health probes. It separates five gates: coordinator health, startup-spawn MCP process collapse, startup-spawn config, agent-scoped primitive HTTP hosting, and watcher-WAL routing. The fixture-backed checker runs in `pnpm doc-sync`, so a healthy isolated host proof cannot hide a mixed live topology with duplicated primitive bundles or missing agent-scoped primitive hosting.
