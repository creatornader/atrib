...
Passing that proof means the host binary can satisfy the local contract in isolation. Live adoption still needs a separate topology report over the host's real process table, config surfaces, and producer activity. Host-specific rollout history and per-gate operational detail (state paths, service labels, per-harness adoption slices) are host-owned runbook material and live outside this repo.

That report lives at [`scripts/report-local-substrate-topology.mjs`](../../scripts/report-local-substrate-topology.mjs). It reads process rows, sanitized harness MCP config summaries, service metadata, an optional supervised route registry, coordinator and runtime health probes, and bounded producer-activity reports, and evaluates fifteen rollout gates before broad-default readiness: route-registry diagnostics, coordinator health, coordinator runtime version freshness, startup-spawn MCP process collapse, startup-spawn config, agent-scoped primitive HTTP hosting, primitive runtime version freshness, context routing coverage, agent-scoped bridge HTTP hosting, bridge wrapper footprint, watcher-WAL routing, knowledge-base receipt join-back, knowledge-base watcher activity, long-lived-agent routing, and long-lived-agent activity. The version gates compare live health reports against the checked-out package versions, so a healthy but stale host cannot pass readiness after a release. Long-lived activity evidence is route-bound: activity only counts for a route when its endpoint matches that route's configured endpoint, and mismatches keep the default-trial gate closed. The fixture-backed checker runs in `pnpm doc-sync`, so `ready_for_default_trial` cannot drift into a prose-only claim.
