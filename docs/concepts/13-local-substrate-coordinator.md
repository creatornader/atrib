...
The measurement reuses the same collector but fails closed unless process footprint, shared HTTP surfaces, coordinator queue/orphan state, watcher receipt join-back, long-lived route health, and long-lived producer activity all pass together. It can write a JSON baseline with `--report <path>`, and its fixture checker runs in `pnpm doc-sync` so `ready_for_default_trial` cannot drift into a prose-only claim. Future versions should extend [D084](../../DECISIONS.md#d084-read-primitive-instrumentation-for-empirical-loop-closure-measurement) rather than create another private health surface.
