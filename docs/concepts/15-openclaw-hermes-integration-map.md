...
| Host-native tool hook or SDK callback | `@atrib/agent` concepts, likely host-specific adapter code | The host exposes tool name, args, result, duration, session ids, and call ids. That is the right proof boundary for native tools. | A wrapped MCP server already signs the same call. |
