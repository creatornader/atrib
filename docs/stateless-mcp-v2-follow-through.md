...
| 0B: compatibility observability           | complete | atribd persists bounded profile, client, protocol, and era observations; health and topology agree; a legacy request after modern traffic emits a structured error event and degrades health; the updater and compatibility monitor reject that regression.                                                                                                                                                                                                                                                                                                             |
| 1A: duplicate-safe writes                 | complete | [D184](../DECISIONS.md#d184-stateless-mcp-writes-use-action-bound-idempotency-keys) defines action-bound keys, durable completed-result replay, indeterminate pending outcomes, seven-day retention, and lock-through-settlement. Released daemon and SDK packages carry the implementation. Live duplicate-write probes against all three operator routes returned the same result and record hash on retry, with one completed idempotency entry per route.                                                                                                           |
| 1B: automatic request carriage            | complete | [D185](../DECISIONS.md#d185-client-sdks-carry-complete-stateless-mcp-request-context) adds one shared metadata builder, default receipt negotiation, explicit write context, W3C and legacy carriers, exposed transport facts, and an independent Python v2 HTTP client. The shared corpus, Python mock daemon, and both released clients pass live atribd reads.                                                                                                                                                                                                       |
| 1C: request-scoped security and lifecycle | complete | [D186](../DECISIONS.md#d186-stateless-mcp-security-and-cancellation-are-request-scoped) applies bearer verification and rate limiting per request, forwards cancellation to reads, preserves write settlement and idempotency after caller abandonment, and rejects connection or client-name authority. Real HTTP tests cover v2 cancellation. Content recall yields at phase boundaries so a closed request is observed before later synchronous phases. A live large-mirror proof recorded one cancellation, one settled-after-cancel result, and zero active calls. |
| 2A: resilience and performance            | complete | Real fixed-endpoint tests hard-kill atribd, retry discovery, tool listing, reads, and negotiated-receipt writes, prove one-record idempotent replay, exercise 32 concurrent contexts and busy-context isolation, pin private TTL carriage, and enforce broad local request-path latency budgets.                                                                                                                                                                                                                                                                        |
...
## Ongoing compatibility window

Keep the legacy adapter during the announced compatibility window. Run
`pnpm monitor:mcp-compatibility -- --endpoint <health-url>` against every
profile health endpoint. Any legacy request after modern traffic degrades
daemon health and fails the monitor. Remove the adapter only after the
configured zero-traffic window completes and the deprecation is announced.

Repeat the installed-daemon duplicate-write, cancellation, restart, and
receipt probes after transport, daemon, recall, or SDK releases.
