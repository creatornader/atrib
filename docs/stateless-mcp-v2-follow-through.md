...
| 2A: resilience and performance            | in progress     | A real fixed-endpoint process-replacement test now hard-kills atribd and proves discovery, reads, negotiated receipts, one-record idempotent write replay, private TTL carriage, 32 concurrent contexts, and progress outside a busy context. Stable latency benchmarks and kill-during-in-flight matrices remain. |
