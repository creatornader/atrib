...
The dogfood topology already converged on one host-owned process. The private `@atrib/primitives-runtime` package (`services/atrib-primitives/`) mounts all seven cognitive-primitive MCP servers in-process over `InMemoryTransport`, routes their 15 physical tools through one MCP server, and exposes that server over stdio, Streamable HTTP, or a stdio-to-HTTP proxy. [D120](../DECISIONS.md#d120-local-substrate-coordinator-keeps-startup-spawn-sidecars-wrapper-owned) proved the process-health story; [D127](../DECISIONS.md#d127-primitive-runtime-health-gates-recall-contract-freshness)-[D130](../DECISIONS.md#d130-primitive-runtime-health-uses-non-mutating-behavioral-probes) built the health-gate stack (recall contract freshness, per-package tool-surface contracts, non-mutating behavioral probes, build/restart/direct-probe updates). The seven-process layout cost spawn storms per harness thread, ppid-based session discovery workarounds ([D083](../DECISIONS.md#d083-harness-session-id-discovery-extends-d078-for-cognitive-primitive-mcp-servers) v2/v3 exist *because* of it), per-generation health gating, and a coordinator; the consolidated runtime dissolved exactly those costs. But the runtime is private, and public users still get the seven-stdio-server default.
...
**Amendment (2026-07-10, chain-fork measurement).** A live-corpus analysis found 1,001 chain forks under the operator mirror directory, with no locking anywhere on the read-tail, sign, append path (570 two-child forks; 517 forked within five seconds; most same-producer, same file). The measurement both sharpens the daemon's write-side claim and bounds it:

- *What the daemon fixes:* the genesis race. One daemon per profile serializes read-tail, sign, append per `context_id`, which removes write races among callers routed through it by construction. The `concurrent-writer-serialization/` conformance family below pins this.
- *The partition axis:* one daemon instance per profile, following the [D120](../DECISIONS.md#d120-local-substrate-coordinator-keeps-startup-spawn-sidecars-wrapper-owned) finding that a single env-bound backend is invalid for multi-profile use. Within a profile, the daemon's tail authority is the whole mirror corpus (every `*.jsonl` in the profile's mirror directory), never one file. Tail resolution must therefore be corpus-scoped; that fix lands producer-side in `resolveChainRoot` (see [the executor charter](chain-tail-corpus-scoped-charter.md)) so every producer inherits it, daemon-routed or not. 20 of 21 inheritance-dead forks had the sibling record in a different mirror file, and 105 of 3,190 contexts span more than one file, so file-scoped tail reading was itself a fork source with zero write concurrency.
- *What the daemon does not fix:* cross-boundary races. A writer that appends to the corpus without routing through the daemon sits outside the serialization boundary, and a context that crosses harness boundaries has records in mirrors owned by different signing identities. The daemon's serialization boundary is exactly that per-identity boundary, so daemon locking never substitutes for explicit context injection per [D135](../DECISIONS.md#d135-delegated-builder-atrib-context-threads-via-orchestrator-injected-explicit-args); corpus-scoped resolution narrows the fork window for outside writers but only daemon routing eliminates it.
...
3. **`context-resolution/`**: full carrier-ladder vectors: explicit argument beats `_meta.atrib` beats `_meta.tracestate` beats `_meta['X-Atrib-Chain']`; an `X-Atrib-Chain`-only request resolves (callers that send only `X-Atrib-Chain` depend on that [§1.5.3](../atrib-spec.md#153-http-fallback-x-atrib-chain) rung; dropping it breaks them silently); missing context on HTTP → typed write-primitive rejection; stdio-shim ladder vectors reusing/extending the [D083](../DECISIONS.md#d083-harness-session-id-discovery-extends-d078-for-cognitive-primitive-mcp-servers) env/file cases; interaction vectors proving `resolveChainRoot` output is unchanged (reuse `spec/conformance/1.2.3/multi-producer/` fixtures as inputs).
...
7. **`concurrent-writer-serialization/`** (added 2026-07-10 from the chain-fork measurement): N concurrent write-primitive requests against one `context_id` through one daemon instance yield a linear chain: zero forks, each record's chain root resolving to its predecessor. Mixed-producer concurrent writes through the same daemon stay linear. A documented-boundary case shows a writer appending to the corpus outside the daemon can still fork, which corpus-scoped resolution narrows but only daemon routing eliminates. Fixtures use injected fixed keys and timestamps for reproducibility.
...
- **Collapse the visible tool surface to two tools (write/read) now.** Rejected: [D079](../DECISIONS.md#d079-the-six-core-cognitive-primitives-atribs-agent-facing-surface) explicitly rejects polymorphic dispatch as the agent-facing surface, and the tool list itself steers agent behavior (a listed `atrib-revise` prompts revision recording; a `ref.kind` enum buried in a schema does not). Surface changes belong to the step-6 rename ADR with its own impact catalog.
