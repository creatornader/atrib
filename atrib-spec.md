...
| URI                                           | Binary | Meaning                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| --------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `https://atrib.dev/v1/types/tool_call`        | `0x01` | An agent invoked or attempted to invoke a tool with input(s). The ordinary post-call path emits it after a successful non-error result. Verifiable-action pre-call mode emits one request record before execution and one terminal outcome record after success, an MCP error or invalid result, or a thrown failure or cancellation. Outside that paired mode, it MUST NOT be emitted when `isError: true`. Default for any active operation against external state.                                                                                                                                                                                                                                                                                                                            |
...
- **Ordering-consistency checks** (verifier-side, categorical, signal not block, per [§3.3](#33-verification-state)). When the verifier can resolve the leaf records it MUST check: (a) if CHAIN_PRECEDES A → B and both are leaves, `index(A) < index(B)`; (b) leaf timestamps are non-decreasing beyond declared `timestamp_granularity`; (c) every resolved leaf's `context_id` equals the checkpoint's `context_id`. A violation of (c) is a hard structural fault, not a soft flag. It MUST also check (d) every prior `session_checkpoint` record on the context appears as a leaf.
...
For consecutive checkpoints K_i → K_{i+1} on one `context_id`: `K_{i+1}.checkpoint.prior_checkpoint` MUST be the record hash of K_i; `K_{i+1}.checkpoint.first_index` MUST equal `K_i.checkpoint.tree_size`; and the leaf sequence `0..K_i.tree_size-1` MUST be identical. This append-only extension is provable by an RFC 6962 §2.1.4 consistency proof from `(K_i.session_root, K_i.tree_size)` to `(K_{i+1}.session_root, K_{i+1}.tree_size)`, the same append-only check the log's witness protocol applies between successive log checkpoints ([§2.9](#29-witnessing-and-cosignatures)).

Two signed checkpoints from the same `creator_key` claiming the same `prior_checkpoint` (or overlapping ranges) with inconsistent roots constitute equivocation evidence against that key. This is the session-scale analogue of log equivocation in [§2.11](#211-cross-log-replication), reported as a categorical verifier fact.
...
_Ladder 1: propagation token._ Implementations MUST resolve the inbound propagation token in this order, taking the first carrier that yields a well-formed token:
...
_Ladder 2: context identity._ Implementations MUST resolve `context_id` in this order:
...
2. **A pre-registered emergency key.** Permitted ONLY when `revocation_reason='compromise'`. The `creator_key` field is the emergency key's public key; `emergency_signed_by` MUST equal `creator_key`. The emergency key MUST have been registered in a directory state anchored to the log BEFORE the revocation record's log index under the same identity claim as `revoked_key`. A signer-controlled record timestamp is not registration-order evidence. This is the only path that survives the case where the legitimate owner has lost access to `revoked_key`.
...
- `valid-compromise-emergency`: revocation signed by an emergency key whose directory registration was anchored before the revocation log index.
- `invalid-wrong-signer`: revocation signed by an unrelated key (rejected).
- `invalid-emergency-not-registered`: emergency-key registration was not anchored before the revocation log index (rejected).
...
A **delegation certificate** is a standalone JCS-canonical object in which a _principal_ Ed25519 key certifies an ephemeral _run_ key with an explicit scope, expiry, and optional session binding. Records signed by the run key occupy the existing `creator_key` slot in both the record ([§1.2.1](#121-field-definitions)) and the 90-byte log entry ([§2.3.1](#231-entry-serialization)). No format changes. The certificate is verifiable offline from the certificate alone; there is no deterministic linkage from a parent secret and no PKI ([§1.4.1](#141-key-format) posture).
...
```jsonc
{
  ...
  "scope": {
    ...
    "cost_policy": { "model_tiers": ["standard"], "max_tokens": 500000 },
  }, // OPTIONAL; §6.7.1 capability envelope schema, verbatim
...
The hash is over the _signed_ bytes, analogous to `record_hash` ([§1.2.3](#123-chain_root-for-genesis-records)). It is the key used by the [§1.11.3](#1113-the-delegation_cert_hash-field) record field, run-key revocation ([§1.11.5](#1115-run-key-revocation)), sidecars, and archive evidence keys.
...
**Depth limit.** This version permits depth ≤ 1. `principal_key` MUST NOT itself be a run key under another certificate known to the verifier; a chained certificate is rejected _as delegation evidence_ (`delegation_depth_exceeded`) and the record falls back to depth 0. Chains are future work behind their own decision record.
...
3. For a valid matching `c`, evaluate its window against caller-owned verifier
   or host time when supplied:
   `(c.not_before ?? 0) <= trusted_time_ms <= c.not_after` →
   `in_window`, `time_basis: "trusted_verifier_time"`, and
   `time_trusted: true`. When the caller supplies no trusted time, the verifier
   MAY compute the same signal against `R.timestamp`, but MUST label it
   `time_basis: "record_timestamp_claim"` and `time_trusted: false`. A
   signer-controlled timestamp MUST NOT satisfy an acceptance policy that
   requires trusted expiry evaluation. An invalid caller time is labeled
   `time_basis: "invalid_trusted_time"` and `time_trusted: false`.
   `c.context_id` absent → `context_bound: null`, else
   `c.context_id === R.context_id` → `context_bound`; when the context genesis
   was signed by `R.creator_key` AND carries `delegation_cert_hash`,
   `genesis.delegation_cert_hash === cert_hash(c)` → `cert_bound`, otherwise
   `cert_bound: null` (the standing state for run keys that joined a
   multi-producer context, [§1.11.6](#1116-multi-producer-contexts)).
4. Consult the directory ([§6.3](#63-verifier-consultation-algorithm)) for `c.principal_key`, not the run key. Run keys never enter the directory; the expected lookup result for a run key is a non-membership proof, and a directory claim found _for the run key itself_ is surfaced as the structural anomaly `run_key_in_directory: true`.
5. Scope check per [§6.7.2](#672-verifier-semantics) semantics against
   `c.scope`: `in_scope` with a `mismatches[]` list naming failed constraints
   (`tool_names`, `event_types`, `max_amount`, `counterparties`,
   `cost_policy.model_tiers`, `cost_policy.max_tokens`, `expires_at`).
   Constraints whose required record, protocol, usage, or trusted-time facts
   are absent MUST appear in `unverifiable[]`; `fully_evaluated` is true only
   when that list is empty. Missing facts never count as successful checks.
   When the principal's directory envelope is available, `attenuation_ok`
   reports whether `c.scope` is a subset of it (a certificate granting what
   the principal's own envelope excludes sets `attenuation_ok: false`); when
   no directory envelope is supplied, `attenuation_ok` is `null`. All scope
   outputs are signals, never invalidation, per
   [§6.7.3](#673-out-of-envelope-is-a-signal-not-invalidation).
...
The reference verifier accepts signed revocation records with their log
indexes, verifies each record and the [§1.9.2](#192-signing-rules) revoker path,
and derives the effective revoked-key set at the target log position. Supplying
an unverified key set remains a caller-owned shortcut, not equivalent evidence.
...
"delegation": {
  ...
  "time_basis": "trusted_verifier_time", // trusted_verifier_time | record_timestamp_claim | invalid_trusted_time | null
  "time_trusted": true,             // boolean | null
  ...
  "scope_check": { "in_scope": true, "fully_evaluated": true, "attenuation_ok": null, "mismatches": [], "unverifiable": [] }, // object | null; null when the cert has no scope
  "revoked": false,                 // boolean | null
  "errors": []                      // e.g. "self_certificate", "principal_signature_invalid", "delegation_depth_exceeded"
}
```

At depth 0 every certificate-derived field is `null` and `errors` is empty
(unless a covering-but-invalid certificate was rejected, per step 2). No field
in this block affects record validity. Consumers that gate action acceptance
SHOULD require `time_trusted: true`, `in_window: true`,
`scope_check.fully_evaluated: true`, `scope_check.in_scope: true`, and the
applicable revocation result.

**Ambiguity rule.** If two valid certificates from _different_ principals cover the same run key in overlapping windows, the verifier MUST surface both and set `delegation_ambiguous: true` rather than choosing. Choosing would be interpretation; surfacing is fact.
...
Producers that opt in stamp `delegation_cert_hash` on the genesis record _only when they sign the context genesis_ (per [§1.11.6](#1116-multi-producer-contexts), a producer joining an existing chain writes no genesis and stamps nothing), write `_local.delegation_cert`, and include the certificate in archive submission when the archive path is configured. **Degradation ([§5.8](#58-degradation-contract)):** every failure (unreadable certificate, malformed JSON, expired certificate at startup, or principal-signature mismatch) is caught and logged with the `atrib:` prefix. Signing proceeds _without_ the genesis field. Records remain valid; the verifier simply sees an uncertified run key (`delegation_unresolved` at worst). Delegation failures never block the primary tool call.
...
| evidenceMode     | string     | Optional | `minimal` or `verifiable-action`. The low-level middleware default is `minimal` for signed-byte compatibility. `verifiable-action` applies the [D166](DECISIONS.md#d166-verifiable-action-mode-commits-request-and-outcome-by-default-in-the-wrapper) preset: hashed tool identity plus salted JCS argument and result commitments. The reference `@atrib/mcp-wrap` integration defaults to this mode.                                                                                                                                                                                                                                                                                                                                                               |
| disclosure       | object     | Optional | Explicit `tool_name`, `args`, and `result` posture dials from [§8](#8-privacy-postures). When supplied, they override `evidenceMode`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
In the ordinary post-call path, after the tool handler completes successfully (i.e., `isError` is false in the response), the middleware MUST construct and sign an attribution record per [§1.2](#12-the-attribution-record) through [§1.4](#14-signing-and-verification). Verifiable-action pre-call mode follows the two-record timing below.
...
**Verifiable-action mode.** A normal post-call path emits one record carrying
the hashed tool identity plus salted argument and result commitments. A
pre-call receipt-injection path cannot put the result into the already-signed
request record. It MUST commit the request record before invoking the handler,
then emit a terminal outcome for success, an MCP `isError` or invalid result,
or a thrown failure or cancellation. The outcome record carries the result commitment and
MUST bind to the request record through both `chain_root` and `informed_by`.
Both records use the same `context_id` and `content_id`. The request uses
`tool_call`. The outcome uses `transaction` only when the tool is in
`transactionTools`; otherwise it also uses `tool_call`. This pair adds no
field or event type. The local sidecar distinguishes
`actionPhase: "request"` from `actionPhase: "outcome"`. See
[D166](DECISIONS.md#d166-verifiable-action-mode-commits-request-and-outcome-by-default-in-the-wrapper).
...
**Note (Tool call failures):** The ordinary and minimal paths emit records only for successful tool calls (`isError: false`). Their failed calls remain unsigned gap nodes in the graph ([§3.2.5](#325-gap-nodes)). Verifiable-action pre-call mode has already committed a request before execution, so it MUST extend that request with a terminal outcome commitment when the handler returns `isError: true`, returns no valid result object, or throws. Attribution failure still follows [§5.8](#58-degradation-contract): it MUST NOT replace or suppress the tool's result or exception.
...
The initial atrib-maintained registry is: `oauth2`, `mcp-oauth`, `aauth`, `x401`, `ap2-vi` (mapped 1:1 from the legacy [§5.5.6](#556-generic-authorization-evidence-blocks) adapters), `human-approval` (per [D118](DECISIONS.md#d118-primary-trace-path-is-a-presentation-rule-over-trace-and-chain): the payload is the human-signed approval record itself; `ref.record_hash` names it, `ref.kind` states where its body is retrievable, and `payload.hash` commits to its canonical bytes), `counterparty-attestation` (out-of-band co-signature receipts that are external evidence per [D098](DECISIONS.md#d098-ap2-receipts-stay-external-evidence-for-cross-attestation) / [D107](DECISIONS.md#d107-ap2-counterparty-attestation-signs-atrib-transaction-bytes)), and `delegation-certificate` (the certificate carrier defined by [§1.11.8](#1118-carriage)). Registered after the initial set, under the same rule: `continuation-packet` per [D142](DECISIONS.md#d142-orchestration-topology-baton-pass-and-join-records-as-attest-conventions); `payments-detection` and `payments-settlement` per [D147](DECISIONS.md#d147-payments-profile-spin-out-from-protocol-core); and `nostr-event` plus `buzz-event` per [D179](DECISIONS.md#d179-nostr-and-buzz-events-are-separate-evidence-profiles). The Nostr profile verifies event IDs and Schnorr signatures. The Buzz profile adds NIP-OA owner authorization while keeping community admission, relay acceptance, audit inclusion, and runtime execution as separate claims.
...
**Conformance.** The envelope conformance corpus lives at `spec/conformance/evidence-envelope/` with ten case families: the five shared envelope families, `continuation-packet/`, `payments-detection/`, `payments-settlement/`, `nostr-event/`, and `buzz-event/`. The Nostr cases pin NIP-01 event ID and signature verification. The Buzz cases pin NIP-OA owner authorization and unresolved relay and runtime boundaries. The generator is `packages/log-dev/scripts/generate-conformance-evidence-envelope.ts`; the reference consumer is `packages/verify/test/conformance-evidence-envelope.test.ts`. Profile-internal semantics remain authoritative in the existing corpora at `spec/conformance/5.5.6/{oauth,aauth,x401}/` and `spec/conformance/ap2-vi-crypto/`, which are referenced, not moved.
...
**Key compromise.** [§1.9](#19-key-rotation-and-revocation) defines the normative key rotation and revocation mechanism. Creators who believe their key has been compromised SHOULD publish a `key_revocation` record per [§1.9.1](#191-revocation-record-format) with `revocation_reason: "compromise"` (and a `successor_key` if rotating to a new key), then submit subsequent records under the new key. Verifiers honor the revocation per [§1.9.3](#193-verifier-semantics): records under the retired key at later log indexes are flagged `revoked_after_revocation`. The signed record timestamp does not determine revocation order. The [§6](#6-key-directory) directory propagates revocations to other agents and verifiers.

#### 5.6.4 Named Identity Profile (Informative)

The reference CLI exposes the existing identity and delegation primitives as
one lifecycle:
...
The command creates or recovers one durable principal, derives stable
principal-namespaced workspace and agent labels, signs those names and roles
as an [§6](#6-key-directory) identity claim, and issues a fresh
context-bound [§1.11](#111-delegation-certificates) run certificate. The local
profile stores no secret. It contains the public identity, the signed claim,
and a locator for the Keychain or caller-managed key file. The output carries
the signed claim and certificate so an offline consumer can verify
`run key → principal → named workspace and agent` without resolving a live
directory. Directory discovery is an explicit publication step.

The name and role claim is self-attested unless another [§6.1](#61-identity-claim-format)
method proves more. Its signature proves that the principal key committed to
the claim. It does not prove a legal identity, employment relationship, or
organization control. A simple interface MUST preserve that distinction.

On a repeat invocation, the reference lifecycle treats the fresh key as a
rotation, not an unrelated replacement. It locks the profile, signs a
[§1.11.5](#1115-run-key-revocation) revocation for the prior run under the
principal, submits it to the configured log, records the accepted log index,
then switches the profile to the successor. Submission failure leaves the
prior active-run state unchanged. Consumers can reload the profile's accepted
revocation view, and protected executors can check it before policy evaluation
and again immediately before the side effect.
...
| tool_request_committed | Immediately before invoking a tool handler when verifiable-action pre-call mode is active       | @atrib/mcp   | Commit the receipt-bearing `tool_call` request record, label the local sidecar `actionPhase: "request"`, and begin asynchronous log submission before the receipt is exposed to the upstream tool.                                                                                                                                                                                                                                                                                               |
| tool_served            | Immediately after a tool handler returns, or after it throws in verifiable-action pre-call mode | @atrib/mcp   | In ordinary mode, construct and sign one record only for `isError: false`. In verifiable-action pre-call mode, commit a linked terminal outcome for success, MCP error or invalid result, or thrown failure or cancellation. The outcome uses `transaction` only when the tool is in `transactionTools`; otherwise it uses `tool_call`. Attach context to returned responses and submit asynchronously per [§5.3.3](#533-record-construction-and-signing) through [§5.3.5](#535-log-submission). |
...
| `actionPhase`           | string | `request` or `outcome` when verifiable-action mode emits a pre-call request and post-call outcome pair. Absent on ordinary single-record tool calls.                                                                                                                                                                                                                                                                                                                                                                        |
...
**Step 9: Check revocation.** Resolve valid `key_revocation` records targeting `K` from the log (the directory MAY index them, but is not the source of truth, per [§1.9.3](#193-verifier-semantics)). Compare log positions, never signer-controlled timestamps:

- If `R.log_index < revocation.log_index`: surface `key_revocation_status: { reason, revoked_at_log_index, since_revocation: false, order_verifiable: true, registry_verified }`. The record predates revocation and retains its original verification state.
- If `R.log_index >= revocation.log_index`: surface `key_revocation_status: { reason, revoked_at_log_index, since_revocation: true, order_verifiable: true, registry_verified }` and mark the record `'revoked_after_revocation'`.
- If either log index is unavailable, surface `key_revocation_status: { reason, revoked_at_log_index, since_revocation: null, order_verifiable: false, registry_verified }`. Do not infer order from either record timestamp.

`registry_verified` is true only when the caller verified the source record
signature and [§1.9.2](#192-revocation-authority) revoker authorization.
Parsing well-shaped revocation claims into a map does not establish that
property.
...
```jsonc
{
  ...
  "key_revocation_status":        null | { "reason": "...", "revoked_at_log_index": ..., "since_revocation": bool | null, "order_verifiable": bool, "registry_verified": bool },
...
- `revocation-applies-pre-revocation-soft`: record accepted before the revocation log index → verifier surfaces `since_revocation: false` and preserves its verification state.
- `revocation-applies-post-revocation-flagged`: record at or after the revocation log index → verifier surfaces `since_revocation: true` and marks `'revoked_after_revocation'`.
- `revocation-order-unverifiable`: record or revocation log index absent → verifier does not infer order from timestamps.
...
**Current-state projection.** A recall implementation MAY project revision
lineages into a bounded current-state view. The projection is a consumer read
model over existing signed records, not a new record type or graph edge. It
SHOULD verify each candidate signature, apply an explicit signer and context
policy, return every active revision head, and mark a lineage with more than
one head as unresolved. It MUST NOT choose a winning branch from timestamp,
mirror order, or signer identity unless the caller supplied a policy that
authorizes that choice. Local signature verification alone does not prove
public-log inclusion, so the response SHOULD state its acceptance basis. The
response SHOULD bound lineage cells, heads per lineage, and exclusion details,
and SHOULD report complete counts plus truncation for each bounded collection.
reference `recall` implementation exposes this as `shape: "state"` per
