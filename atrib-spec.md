...
Each signature line begins with `— ` (em-dash, space), followed by the key name, a space, the hex key ID, a `+`, and the base64-encoded 64-byte signature over the note text (the body including its trailing newline).
...
The `__unsigned__` sentinel key is present in the output only if gap nodes received non-zero weight under the policy. Its presence signals to the merchant that some share of value is attributed to unsigned contributions, the merchant's responsibility to decide how to handle.
