...
**Status (2026-07-10):** Execution started on branch `feat/atribd-daemon` in a dedicated build session. The gate above is reframed: it binds the transport binding, not the start of work. The daemon core builds now with the transport isolated behind an adapter interface (the fallback path the gate already names); when the Tier-1 SDK ships stateless-transport support, the adapter internals swap, and if the 2026-10-06 date lapses first, the operator decides between staying on the adapter and holding promotion. The daemon core, conformance corpus, and health gates are done work either way.
