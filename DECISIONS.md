...
- *Documentation.* Each profile gets a 1-2 page operator runbook in `docs/operator/hsm-<profile>.md` (operator-internal first; promoted to public at first non-operator user).
...
D040 is reserved for the harness reference implementation ADR (per Phase 3.5 of the gap-closure plan: scope of `@atrib/recall`, why it's informative-not-normative). It will be authored when @atrib/recall moves from operator-internal staging to the public `packages/recall/` directory.
