...
## D147: atribd is the public stateless-native local daemon for the primitive runtime
...
1. Signed records are byte-identical across standalone stdio servers, the daemon HTTP surface, and the alias mounts, with identical `_local.producer` labels. The daemon calls the same handler code paths and never reimplements chain selection; `resolveChainRoot` ([D067](#d067-multi-producer-chain-composition-precedence-contract)) stays the single source of truth, so the corpus-scoped tail fix lands producer-side and the daemon inherits it.
