...
**Aggregated MCP runtime shipped 2026-06-11:** `@atrib/primitives-runtime` adds a private local `atrib-primitives` stdio binary for dogfood harnesses that need process-count reduction before a full coordinator migration. It mounts the seven public primitive packages in process and exposes their 15 physical MCP tools through one MCP server. The standalone public primitive packages stay unchanged and publishable. This reduces the per-thread atrib primitive process target from seven OS child processes to one when a host config points at `services/atrib-primitives/dist/index.js`. It is not a coordinator: it does not own signer policy, WAL commit, receipt join-back, queue health, or cross-harness supervision. The protocol proof is `services/atrib-primitives/test/mcp-protocol.test.ts`, which lists all 15 tools and routes a recall call through the combined server.
