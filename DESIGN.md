...
| Action `#/action/<record_hash>`     | Verify one signed record and decide where to inspect next.                             | The receipt panel is strong, but proof checks still rely on what the log accepted rather than showing a full independent verifier path.                      | Subject, event summary, signer, inclusion/proof checks, session/identity/trace links, raw JSON. | Add explicit "what this proves" and "what this does not prove" copy. Clarify missing directory claim as optional, not a warning. |
...
| Session `#/session/<context_id>`    | Read one agent workflow as a sequence and graph.                                       | Fallback states are honest but too implementation-heavy, especially when graph-node errors or large sessions produce sparse views.                           | Session summary, record count/time span, graph when useful, records table, fallback reason.     | Rewrite graph fallback copy around user meaning: graph unavailable, log records still trustworthy.                               |
