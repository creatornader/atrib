...
### Packet 5: Firecrawl-native demo shape

Scope: reshape the hosted demo so it follows Firecrawl's public demo pattern:
web source, clean context, structured extraction, bounded crawl, and downstream
policy gate.

Allowed files or surfaces:

- `packages/integration/examples/firecrawl-web-ingestion/**`
- `packages/integration/test/firecrawl-web-ingestion-live-demo.test.ts`
- `proof-packets/firecrawl-web-ingestion/**`
- `proof-packets/CONTACT_DRAFTS.md`
- Fly app `atrib-firecrawl-ingestion-demo`
...
- `demo_surface` contract in `/api/config`
- Pipeline-first hosted UI
- Test coverage for the Firecrawl demo category
- Hosted screenshot and accepted hosted run receipt

Objective checks:

- Focused Vitest for Firecrawl live demo and MCP proof packets
- Integration typecheck
- Prettier, `pnpm doc-sync`, typo scan, `git diff --check`
- Local Playwright screenshots before and after a proof run
- Hosted `/health`, `/api/config`, screenshot, and one accepted hosted proof run

Human gates:

- Any GitHub comment, email, DM, Discord message, or X post
...
| 9    | Reshaped the Firecrawl demo from receipt console into source-to-context pipeline.          | `pnpm --filter @atrib/integration... build`; focused Vitest; integration typecheck; Prettier; `pnpm doc-sync`; typos; `git diff --check`; local Playwright screenshots; UI-run fixture proof; Fly deploy; hosted `/health`; hosted `/api/config`; hosted screenshot; hosted proof run. | Accepted: local UI rendered before and after proof; hosted config reports `grounding-pipeline`; hosted proof `fc-mr0vn48k-vh8axe` accepted with tool indexes `68465` through `68468` and control indexes `68469`, `68470`. | None.             | Commit and merge.                       |
