...
Extend the Firecrawl proof packet so it shows how signed web-ingestion evidence
can feed a downstream policy decision before a sensitive action runs.
...
Scope: add a deterministic policy decision artifact to the Firecrawl packet.
...
- Fixture test coverage that checks the decision artifact and redaction boundary
