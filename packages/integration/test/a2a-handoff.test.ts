...
describe('A2A handoff proof', () => {
  it('verifies an A2A v1 data part before signing a receiving-agent follow-up', async () => {
    ...
    expect(result.agent_card.url).toBe('https://a2a.example.test/atrib-handoff/jsonrpc')
    expect(result.agent_card.protocol_binding).toBe('JSONRPC')
    ...
    expect(result.a2a.return_immediately).toBe(false)
    expect(result.a2a.response_kind).toBe('message')
    expect(result.a2a.response_part_content_cases).toEqual(['text', 'data'])
    expect(result.a2a.packet_part_content_case).toBe('data')
