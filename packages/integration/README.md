...
2. **Runnable framework examples** showing how to wire atrib into every supported MCP host and runtime boundary: Claude Agent SDK, Cloudflare Agents, Vercel AI SDK, LangChain JS, LangGraph Store memory, LangGraph Python checkpointing, LlamaIndex.TS memory, LlamaIndex Python memory, Letta memory, A2A handoff evidence, Google ADK TypeScript and Python plugin callbacks, Google ADK Python and TypeScript decision-ledger proofs, Google stack chain proof plus visual workbench, OpenAI Agents runtime receipts, OpenAI Responses tool-call receipts, Mastra MCP and workflow runtime receipts, Microsoft Agent Framework workflow receipts, browser workflow receipts, Browserbase Stagehand, Firecrawl, and OpenETR proof artifacts, ActiveGraph runtime-log proofs, the reference runtime-log JSONL source, Buzz observer-frame windows, the fixture-level Buzz cross-control-plane composition, the source-neutral open runtime composition, the dogfood Agent Bridge runtime-log proof, the secondary runtime-log adapter pair, runtime-log verifier UX, Brief dcbench evidence, proof-log receipts, evidence-packet verification, trace repair suspect ranking, Graphiti, the signer-proxy sandbox pattern, action-control gate proofs, local x401 proof-gate propagation, plus the standalone end-to-end demo.
...
| **A2A handoff evidence**             | [`examples/a2a-handoff/`](examples/a2a-handoff/)                                                         | A real `@a2a-js/sdk` v1 AgentCard, JSON-RPC client, and request handler carry a signed Agent Card plus atrib evidence packet in a data-valued A2A `Part`; the receiver verifies it before signing a follow-up with `informed_by`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| **Cloudflare x402 paid agent**       | [`examples/cloudflare-agents/paid-x402-action-gate/`](examples/cloudflare-agents/paid-x402-action-gate/) | A credential-free Cloudflare Agents shaped fixture uses `@atrib/action-gate` to allow one paid MCP read, signs the outcome, and binds hash-only x402 lifecycle facts to the decision and outcome. The public packet lives at `proof-packets/cloudflare-x402-paid-agent/` and does not require Gateway beta access.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| **Browser workflow receipt**         | [`examples/browser-workflow/`](examples/browser-workflow/)                                               | Deterministic, browser-use-hosted, and Stagehand local-session smokes sign observe, click, fill, and submit as hash-only atrib records while local sidecars keep page, selector, form, and result material. The browser-use smoke uses a real `BrowserSession`; the Stagehand smoke launches a real local v4 `@browserbasehq/stagehand` browser, creates the SDK through its supported factory, captures a protocol snapshot and page text, then uses its local browser protocol for deterministic actions.                                                                                                                                                                                                                                                                                                                        |
| **Browserbase Stagehand proof**      | [`examples/browserbase-stagehand/`](examples/browserbase-stagehand/)                                     | Browserbase MCP shaped `start`, `navigate`, `observe`, `act`, `extract`, and `end` calls through `@atrib/mcp-wrap`. Fixture mode stays local for tests. Live mode can wrap hosted Browserbase Streamable HTTP MCP or self-hosted STDIO, submits narrow records to the public log, and writes `proof-packets/browserbase-stagehand/` while keeping session replay, selectors, snapshots, and form values hash-only. The live demo serves an agent-ready WebMCP target app plus proof console from `examples/browserbase-stagehand/live-demo/`.                                                                                                                                                                                                                                                                                      |
...
| **OpenETR transfer proof**           | [`examples/openetr-transfer/`](examples/openetr-transfer/)                                               | OpenETR-shaped `openetr_issue`, `openetr_transfer_initiate`, `openetr_transfer_accept`, and `openetr_query_state` calls through `@atrib/mcp-wrap`. Fixture mode signs the transfer-control chain and stops before title recognition. Source-backed public mode runs the pinned `trbouma/openetr` Python implementation, publishes to configured public relays, checks exact event availability, verifies external or operator-demo title-authority evidence, verifies external or operator-demo legal/MLETR evidence, writes a source checklist, executes recognition, and can submit accepted atrib records plus control records to the public log.                                                                                                                                                                               |
...
| **Buzz observer runtime log**        | [`examples/buzz-observer-runtime-log/`](examples/buzz-observer-runtime-log/)                             | Runnable proof for the public `@atrib/runtime-log/buzz` source. Signed Buzz NIP-AO kind 24200 telemetry becomes a bounded process-level runtime-log manifest through a host-owned decrypt callback. The source accepts live or archived captures, verifies Nostr events, commits the complete decrypted object, checks owner and agent tags, detects gaps, duplicates, and out-of-order capture, and keeps relay, audit, and tool-execution claims separate. ACP sessions remain projections because current `buzz-acp` assigns sequence numbers process-wide.                                                                                                                                                                                                                                                                     |
| **Buzz cross-control-plane fixture** | [`examples/buzz-cross-control-plane/`](examples/buzz-cross-control-plane/)                               | Hermetic fixture that composes signed Buzz-shaped ACP observer frames, an `@atrib/sdk` paired action, [D168 coverage](../../DECISIONS.md#d168-coverage-manifests-make-capture-scope-verifiable), explicitly mapped operating events, receiving-side handoff verification, a recomputed bounded view, and a one-time protected non-Buzz effect. It does not claim a live Buzz supervision endpoint, relay or audit inclusion, complete capture, or arbitrary result truth.                                                                                                                                                                                                                                                                                                                                                          |
| **Open runtime composition**         | [`examples/open-runtime-composition/`](examples/open-runtime-composition/)                               | Local source-neutral composition over one generated Codex rollout. It atomically accepts a [D183](../../DECISIONS.md#d183-runtime-observation-adapters-separate-reading-from-durable-acceptance) batch and cursor, keeps observer and mapped-agent identity separate, requires an explicit signed semantic mapping and bounded view receipt, verifies a real `@atrib/sdk` request/outcome pair plus signed [D168](../../DECISIONS.md#d168-coverage-manifests-make-capture-scope-verifiable) membership, and lets a separate receiver gate one selected effect. Hostile arms omit exact membership or map the observation into the wrong task and execute no effect. It does not claim a live external session, complete capture, runtime-vendor provenance, telemetry as execution, arbitrary result truth, or a deployed product. |
...
| **mem0 action evidence**             | [`examples/mem0/`](examples/mem0/)                                                                       | Real `mem0ai/oss` mutation assurance with a pre-action identity-scope policy and read-back postconditions, plus the existing add/search boundary smokes for TypeScript OSS, `MemoryClient`, and Python OSS. Public records stay hash-only while local sidecars keep memory bodies.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
artifact paths. Hosted Browserbase fresh runs can return temporary
model-capacity errors, so the deployed demo must show failed runs plainly and
rate-limit retries. Firecrawl has a deployed fixed-input demo at
`https://atrib-firecrawl-ingestion-demo.fly.dev/`. It does not accept arbitrary
URLs, queries, crawl depths, or crawl limits because hosted crawling needs
strict abuse and cost controls.
OpenETR now has a deterministic fixture path and a source-backed public proof
path. The fixture path stays local for tests and stops before title recognition.
The source-backed path can publish exact OpenETR events to configured public
relays, check event availability, ingest a recognized TTA Nostr event, generate
an operator-demo TTA event, verify legal/MLETR attestations, write an MLETR
source checklist, execute recognition, and submit accepted atrib records plus
control records to the public log.
...
Write the checked public packet with:

```bash
pnpm --filter @atrib/integration open-x401-credential-packet
```

That command writes `proof-packets/x401-open-credential-e2e/` with verifier output, provenance, a redaction manifest, and a short reader README. It is the durable artifact for public protocol evidence. It does not submit records to `log.atrib.dev`, because the proof uses local credential material and keeps raw headers hash-only.
...
Use `-- --require-native-current-spec --require-current-evidence` when a check should fail unless the released SDK exposes the hosted current header names and the resulting atrib evidence verifies.

Check the Proof credential verifier handoff into x401 evidence with:

```bash
pnpm --filter @atrib/integration proof-vc-common-x401-interop -- --require-current-evidence --require-proof-vc-common
```

The default path uses a local fixture verifier. The live path is provider-specific: it uses `@proof.com/proof-vc-server@0.3.0` to verify a real Proof `vp_token`, checks that the credential key-binding nonce matches the x401 nonce, and then feeds only caller-owned verifier outcomes into x401 evidence. Run that live path when the question is "does a Proof-issued credential and Proof verifier feed atrib's x401 evidence shape?" It is not needed for the public x401 protocol claim, because the open credential verifier already proves the current-spec challenge, response, result, credential-verifier, and signed-action chain without a Proof platform account.
...
The report classifies `proof/x401`, `proof/x401-node`, `proof/proof-vc-common`, `proof/proof-vc-web`, and `proof/verifier-vcp-demo` separately. Only a current-spec x401 wire SDK is allowed as a public package or core runtime dependency. Proof credential request and verifier packages, including `@proof.com/proof-vc-common` and `@proof.com/proof-vc-server`, stay opt-in fixture helpers. They can feed caller-owned verifier outcomes into x401 evidence, but they are not the x401 wire implementation. `proof-vc-web` is a browser credential collection UX reference, not an x401 wire SDK. `verifier-vcp-demo` is a product-flow reference and can become an opt-in fixture after it uses current x401 wire names.
...
- **`test/hostile-operating-graph.test.ts`** (6 tests), composed hostile-case
  coverage for the open operating graph. It detects checkpoint rollback,
  classifies matching result bodies as uncorroborated and mismatches as
  evidence-inconsistent, rejects replayed permits and revoked credentials,
  refuses withheld required bodies, preserves conflicting state heads, and
  detects a successful source action omitted from an event projection.
...
- **`test/ap2-local-participant.test.ts`** (3 tests), local AP2 participant artifact generation. It proves an AP2 result plus conforming AP2 / VI evidence can be rehydrated into live interop artifacts and paired with a counterparty-signed atrib transaction record, including upstream AP2 full-chain mandate normalization.
...
- **`test/x401-evidence-e2e.test.ts`** (6 tests), x401 authorization evidence propagation. It proves x401 remains separate from payment detection, runs the local proof-gate harness through archive projection without exposing private credential payloads, composes x401 with AAuth plus separate x402 payment detection, runs a two-endpoint proof-gate propagation chain with separate request ids and signed action records, runs an open local JWT VC / VP verifier path without a Proof account, and checks the committed x401 proof packet stays sanitized.
...
- **`test/ap2-vi-reference-artifacts.test.ts`** (2 tests), upstream AP2 plus VI reference artifacts. It preserves the generated artifact bytes and rejects the L2 credential's repeated digest references under SD-JWT 0.20.0. The same test also covers the opt-in advisory conformance policy while AP2 receipts and counterparty-signed atrib transaction bytes continue to verify.
...
- **`test/a2a-handoff.test.ts`** (2 tests), A2A handoff evidence coverage. It proves a real `@a2a-js/sdk` v1 JSON-RPC client and request handler can carry a signed Agent Card plus atrib handoff packet in a data-valued A2A `Part`, then the receiver verifies the Agent Card signature, signer, context, body commitment, freshness, and log inclusion before signing an `informed_by` follow-up. The second test checks the proof's operation timings.
...
- **`test/cloudflare-x402-paid-agent-proof.test.ts`** (1 test), Cloudflare x402 paid-agent proof coverage. It proves a paid MCP request can be allowed by policy, executed once, and bound to hash-only x402 lifecycle facts without raw payment headers or Gateway logs in public output.
...
- **`test/mcp-platform-proof-packets.test.ts`** (6 tests by default, 7 when `OPENETR_SOURCE_DIR` is set, 9 when public OpenETR relay env is set), Browserbase Stagehand, Firecrawl, and OpenETR proof coverage. It proves fixture runs go through `@atrib/mcp-wrap`, verify each signed record, write proof artifacts, and keep private session, selector, query, URL, content, crawl job, object digest, controller, relay, and event-id material out of public outputs. With `OPENETR_SOURCE_DIR`, it also proves the OpenETR packet can bind to a real upstream Python issue, transfer-initiate, transfer-accept, and query run over a local WebSocket Nostr relay. With public relay env, it proves exact OpenETR event availability plus legacy fixture evidence and operator-demo title-authority/legal evidence.
- **`test/browserbase-stagehand-live-demo.test.ts`** (8 tests), Browserbase proof console coverage. It checks proof-runner output parsing, Browserbase workflow metadata, receipt link shaping, rate limits, deployment guards, and the HTTP config plus run API using a fixture runner.
- **`test/firecrawl-web-ingestion-live-demo.test.ts`** (6 tests), Firecrawl ingestion-gate console coverage. It checks fixed-input workflow metadata, policy decision rendering, rate limits, deployment guards, and the HTTP config plus run API using a fixture runner.
...
- **`../runtime-log/test/buzz.test.ts`** (11 tests), public Buzz observer-source coverage. It proves complete signed windows verify, live event loading works without a file, full decrypted objects remain committed across unknown fields, event commitments ignore archive formatting, signatures fail before decryption, oversized payloads fail, missing or duplicate frames fail closed by default, reported gaps stay explicit, out-of-order capture remains visible after logical sorting, interleaved ACP sessions do not become false gaps, and owner-recipient mismatches fail.
- **`test/buzz-cross-control-plane.test.ts`** (3 tests), private cross-control-plane fixture coverage. It proves a contiguous signed Buzz-shaped ACP window composes with an `@atrib/sdk` request/outcome pair, [D168 coverage](../../DECISIONS.md#d168-coverage-manifests-make-capture-scope-verifiable), explicit accepted-state and handoff mapping, receiver-side packet and operating-view verification, and one protected non-Buzz effect. Its hostile arm changes only supplied result evidence and proves the original record signature remains valid while the receiver blocks. Its gap arm rejects a missing observer frame before the producer effect runs.
- **`test/open-runtime-composition.test.ts`** (3 tests), source-neutral application composition coverage. It proves a generated Codex rollout batch enters the atomic observation journal before explicit semantic mapping, then composes a bounded view receipt, paired SDK records, signed [D168](../../DECISIONS.md#d168-coverage-manifests-make-capture-scope-verifiable) membership, and an independent selected-effect verdict. Missing outcome membership and wrong-task mapping both block with zero effect.
