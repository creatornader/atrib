...
| **mem0 boundary**              | [`examples/mem0/`](examples/mem0/)                           | `Memory.add()` and `Memory.search()` boundary wrapper. It preserves mem0-shaped return values, signs hash-only records, and keeps raw memory bodies in local sidecars.                       |
