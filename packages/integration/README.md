...
- **`test/mcp-platform-proof-packets.test.ts`** (2 tests), Browserbase Stagehand and Firecrawl proof coverage. It proves both fixture runs go through `@atrib/mcp-wrap`, verify each signed record, write proof artifacts, and keep private session, selector, query, URL, content, and crawl job material out of public outputs.
- **`test/browserbase-stagehand-live-demo.test.ts`** (3 tests), Browserbase proof console coverage. It checks proof-runner output parsing, receipt link shaping, and the HTTP config plus run API using a fixture runner.
