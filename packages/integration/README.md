...
- **`test/mcp-platform-proof-packets.test.ts`** (5 tests), Browserbase Stagehand and Firecrawl proof coverage. It proves both fixture runs go through `@atrib/mcp-wrap`, verify each signed record, write proof artifacts, and keep private session, selector, query, URL, content, and crawl job material out of public outputs.
