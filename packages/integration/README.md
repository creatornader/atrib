...
relays, check event availability, sign fixture title-authority and legal/MLETR
attestations, execute recognition, and submit accepted atrib records plus
...
- **`test/mcp-platform-proof-packets.test.ts`** (6 tests by default, 7 when `OPENETR_SOURCE_DIR` is set, 8 when public OpenETR relay env is set), Browserbase Stagehand, Firecrawl, and OpenETR proof coverage. It proves fixture runs go through `@atrib/mcp-wrap`, verify each signed record, write proof artifacts, and keep private session, selector, query, URL, content, crawl job, object digest, controller, relay, and event-id material out of public outputs. With `OPENETR_SOURCE_DIR`, it also proves the OpenETR packet can bind to a real upstream Python issue, transfer-initiate, transfer-accept, and query run over a local WebSocket Nostr relay. With public relay env, it proves exact OpenETR event availability plus signed fixture title-authority and legal/MLETR attestations.
