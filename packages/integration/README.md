...
| **OpenETR transfer proof**        | [`examples/openetr-transfer/`](examples/openetr-transfer/)                           | OpenETR-shaped `openetr_issue`, `openetr_transfer_initiate`, `openetr_transfer_accept`, and `openetr_query_state` calls through `@atrib/mcp-wrap`. Fixture mode signs the transfer-control chain. Source-backed mode runs the pinned `trbouma/openetr` Python implementation against a local WebSocket Nostr relay, writes `proof-packets/openetr-transfer/`, probes public relay availability when `OPENETR_PUBLIC_RELAY_URLS` is set, and signs a control-record policy decision that stops before recognized title transfer until authority and legal evidence exist. |
...
OpenETR stays local-only for now. The default packet is a deterministic fixture;
the source-backed packet runs the pinned upstream Python implementation against
a local WebSocket Nostr relay. Public relay and title-transfer authority proof
remain future work.
...
- **`test/mcp-platform-proof-packets.test.ts`** (6 tests by default, 7 when `OPENETR_SOURCE_DIR` is set), Browserbase Stagehand, Firecrawl, and OpenETR proof coverage. It proves fixture runs go through `@atrib/mcp-wrap`, verify each signed record, write proof artifacts, and keep private session, selector, query, URL, content, crawl job, object digest, controller, relay, and event-id material out of public outputs. With `OPENETR_SOURCE_DIR`, it also proves the OpenETR packet can bind to a real upstream Python issue, transfer-initiate, transfer-accept, and query run over a local WebSocket Nostr relay.
