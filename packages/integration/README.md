...
2. **Runnable framework examples** showing how to wire atrib into every supported MCP host and runtime boundary: Claude Agent SDK, Cloudflare Agents, Vercel AI SDK, LangChain JS, LangGraph Store memory, LlamaIndex.TS memory, Letta memory, A2A handoff evidence, Google ADK plugin callbacks, OpenAI Agents runtime receipts, OpenAI Responses tool-call receipts, Mastra MCP and workflow runtime receipts, browser workflow receipts, Brief dcbench evidence, proof-log receipts, evidence-packet verification, trace repair suspect ranking, Graphiti, the signer-proxy sandbox pattern, plus the standalone end-to-end demo.
