...
2. **Runnable framework examples** showing how to wire atrib into every supported MCP host and runtime boundary: Claude Agent SDK, Cloudflare Agents, Vercel AI SDK, LangChain JS, LangGraph Store memory, LlamaIndex.TS memory, A2A handoff evidence, Google ADK plugin callbacks, browser workflow receipts, Brief dcbench evidence, evidence-packet verification, Graphiti, the signer-proxy sandbox pattern, plus the standalone end-to-end demo.
