...
| **Google stack chain proof**    | [`examples/google-stack-chain/`](examples/google-stack-chain/)                       | A local proof ladder that composes AP2 / VI receipt verification, A2A signed handoff evidence, and Google ADK Python plugin signing into one JSON summary for verifier and outreach review.                                                                                                                                                                               |
...
- **`test/google-stack-chain.test.ts`** (opt-in), Google stack chain proof coverage. When `ATRIB_RUN_GOOGLE_STACK_CHAIN_PROOF=1` is set, it composes AP2 / VI live interop, A2A handoff evidence, and Google ADK Python plugin signing into one proof summary and keeps the managed-Google caveats explicit.
