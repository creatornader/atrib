...
| **Buzz observer runtime log**        | [`examples/buzz-observer-runtime-log/`](examples/buzz-observer-runtime-log/)                               | Signed Buzz NIP-AO kind 24200 telemetry converted into a bounded process-level runtime-log manifest through a host-owned decrypt callback. The adapter verifies Nostr events, checks owner and agent tags, detects gaps, duplicates, and out-of-order capture, and keeps relay, audit, and tool-execution claims separate. ACP sessions remain projections because current `buzz-acp` assigns sequence numbers process-wide.                                                                                                            |
...
- **`test/buzz-observer-runtime-log.test.ts`** (6 tests), Buzz observer-frame runtime-log coverage. It proves complete signed windows verify, missing frames fail closed by default, reported gaps stay explicit, out-of-order capture remains visible after logical sorting, interleaved ACP sessions do not become false gaps, and owner-recipient mismatches fail.
