...
2. **Runnable framework examples** showing how to wire atrib into every supported MCP host and runtime boundary: Claude Agent SDK, Cloudflare Agents, Vercel AI SDK, LangChain JS, LangGraph Store memory, LlamaIndex.TS memory, A2A handoff evidence, Google ADK plugin callbacks, OpenAI Agents runtime receipts, Mastra MCP runtime receipts, browser workflow receipts, Brief dcbench evidence, proof-log receipts, evidence-packet verification, trace repair suspect ranking, Graphiti, the signer-proxy sandbox pattern, plus the standalone end-to-end demo.
...
| **OpenAI Agents runtime**      | [`examples/openai-agents-runtime/`](examples/openai-agents-runtime/) | A real `@openai/agents` `Agent`, `run()` loop, and local `tool()` function emit a signed hash-only atrib record from the SDK `agent_tool_end` lifecycle event while a scripted model keeps the proof local and credential-free.                                                                                                |
| **Mastra runtime receipt**     | [`examples/mastra-runtime/`](examples/mastra-runtime/)               | A real `@mastra/mcp` `MCPClient` connects to a real `MCPServer` over stdio, executes a namespaced Mastra `createTool()` tool, signs one hash-only atrib record, and keeps raw procurement material in local sidecars.                                                                                                          |
| **Browser workflow receipt**   | [`examples/browser-workflow/`](examples/browser-workflow/)           | Deterministic and browser-use-hosted browser-action smokes sign observe, click, fill, and submit as hash-only atrib records while local sidecars keep page, selector, form, and result material. The browser-use smoke uses a real `BrowserSession`, not an autonomous LLM agent loop.                                         |
...
- **`test/openai-agents-runtime-receipt.test.ts`** (2 tests), OpenAI Agents runtime receipt coverage. It proves a real `@openai/agents` `Agent`, `run()` loop, and local `tool()` function can sign a hash-only atrib record from the SDK `agent_tool_end` lifecycle event while local sidecars keep the tool payload.
- **`test/mastra-runtime-receipt.test.ts`** (2 tests), Mastra runtime receipt coverage. It proves a real `@mastra/mcp` `MCPClient` can execute a Mastra `MCPServer` tool over stdio, sign a hash-only atrib record, and keep raw tool payloads in local sidecars.
- **`test/browser-workflow-receipt.test.ts`** (3 tests), browser workflow receipt coverage. It proves observe, click, fill, and submit actions sign hash-only records in one chain while local sidecars retain private page and form material, including a real browser-use `BrowserSession` smoke.
