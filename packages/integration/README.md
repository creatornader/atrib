...
| **Google ADK Python plugin**    | [`examples/google-adk-python/`](examples/google-adk-python/)                         | A real Python `google-adk==2.1.0` `InMemoryRunner`, `BasePlugin`, and `FunctionTool` call path emits a signed hash-only atrib record while local sidecars keep ADK app, session, user, invocation, function-call id, arguments, and result material inspectable.                                                                                                          |
| **Google stack chain proof**    | [`examples/google-stack-chain/`](examples/google-stack-chain/)                       | A local proof chain that links AP2 / VI receipt verification, A2A signed handoff evidence, and Google ADK Python plugin signing through verifier-resolved `informed_by` records, with a deterministic snapshot, BigQuery Agent Analytics-shaped local fixture, and a static visual workbench.                                                                             |
...
- **`test/google-adk-python-attribution.test.ts`** (opt-in), Google ADK Python plugin coverage. When `ATRIB_RUN_GOOGLE_ADK_PYTHON_SMOKE=1` is set, it imports `google-adk==2.1.0` through `uv`, runs a real Python `InMemoryRunner` with `BasePlugin` and `FunctionTool`, then verifies one hash-only atrib record for the Python tool callback.
- **`test/google-stack-chain.test.ts`** (opt-in), Google stack chain proof coverage. When `ATRIB_RUN_GOOGLE_STACK_CHAIN_PROOF=1` is set, it links AP2 / VI live interop, A2A handoff evidence, and Google ADK Python plugin signing through verifier-resolved `informed_by`, pins the deterministic snapshot hashes, and keeps the managed-Google caveats explicit.
- **`test/google-stack-chain-visual.test.ts`** (1 test), Google stack chain visual coverage. It serves the static workbench, opens it in Playwright, selects the ADK Python callback stage, and verifies the selected record inspector plus analytics row stay in sync.
