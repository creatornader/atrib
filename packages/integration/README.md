...
2. **Runnable framework examples** showing how to wire atrib into every supported MCP host and runtime boundary: Claude Agent SDK, Cloudflare Agents, Vercel AI SDK, LangChain JS, LangGraph Store memory, LangGraph Python checkpointing, LlamaIndex.TS memory, LlamaIndex Python memory, Letta memory, A2A handoff evidence, Google ADK TypeScript and Python plugin callbacks, Google ADK decision-ledger proof, Google stack chain proof plus visual workbench, OpenAI Agents runtime receipts, OpenAI Responses tool-call receipts, Mastra MCP and workflow runtime receipts, Microsoft Agent Framework workflow receipts, browser workflow receipts, ActiveGraph runtime-log proofs, the reference runtime-log JSONL source, the dogfood Agent Bridge runtime-log proof, the secondary runtime-log adapter pair, runtime-log verifier UX, Brief dcbench evidence, proof-log receipts, evidence-packet verification, trace repair suspect ranking, Graphiti, the signer-proxy sandbox pattern, plus the standalone end-to-end demo.
...
| **Google ADK decision ledger**  | [`examples/google-adk-decision-ledger/`](examples/google-adk-decision-ledger/)       | A real `@google/adk` `InMemoryRunner`, `BasePlugin`, and `FunctionTool` path signs an allow or refuse decision from `beforeToolCallback`, then signs the allowed tool outcome from `afterToolCallback` with the decision record in `informed_by`. Confirmation fixtures pin the local binding and mismatch contract.                                                      |
| **Google ADK Python plugin**    | [`examples/google-adk-python/`](examples/google-adk-python/)                         | A real Python `google-adk==2.3.0` `InMemoryRunner`, `BasePlugin`, and `FunctionTool` call path emits a signed hash-only atrib record while local sidecars keep ADK app, session, user, invocation, function-call id, arguments, and result material inspectable.                                                                                                          |
| **Google stack chain proof**    | [`examples/google-stack-chain/`](examples/google-stack-chain/)                       | A local proof chain that links AP2 / VI receipt verification, A2A signed handoff evidence, a Google ADK JS allow decision, and the ADK JS tool outcome through verifier-resolved `informed_by` records, with a deterministic snapshot, BigQuery Agent Analytics-shaped local fixture, Cloud Run-backed runtime path, and live-first visual workbench.                     |
...
- **`test/google-adk-decision-ledger.test.ts`** (2 tests), Google ADK decision-ledger coverage. It proves a real `@google/adk` `beforeToolCallback` can sign allow and refuse decisions before dispatch, the allowed outcome cites the decision record, refused calls do not execute the tool body, and stale confirmation bindings fail closed.
