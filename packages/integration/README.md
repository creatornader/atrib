...
| **mem0 boundary**              | [`examples/mem0/`](examples/mem0/)                           | `Memory.add()` and `Memory.search()` boundary wrapper plus a real `mem0ai/oss` compatibility smoke. It preserves mem0-shaped values, signs hash-only records, and keeps raw memory bodies in local sidecars. |
