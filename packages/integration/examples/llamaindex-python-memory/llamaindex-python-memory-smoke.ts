...
export async function runLlamaIndexPythonMemorySmoke(): Promise<SmokeResult> {
  ...
  return {
    ...
    caveats: [
      ...
      'It does not call an LLM, run a full LlamaIndex agent workflow, persist SQL history outside the default local store, or prove maintainer interest.',
      'Outreach still needs operator approval after a same-day LlamaIndex route refresh.',
    ],
  }
}
