...
`createInProcessLocalSubstrateCoordinator()` is the first opt-in prototype for startup-spawn and watcher-WAL trials. It exposes a coordinator transport without creating a daemon, signs only when the unsigned body's `creator_key` matches the coordinator signer, returns the real `record_hash` and receipt token, accepts caller-owned health counters, and leaves log submission optional for tests. Its default harness scope is still `startup-spawn`; watcher-WAL callers opt in through `supportedHarnessClasses: ['watcher-wal']` and use `operation: 'enqueue_record_and_join_receipt'` so receipt join metadata stays explicit. Long-lived agents still have shadow evidence only.
