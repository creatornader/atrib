...
This is the **server-side half** of the atrib protocol: the package tool creators install. If you're building an agent that _calls_ MCP tools, you want [`@atrib/agent`](../agent/README.md) instead.
...
That's the entire integration. Every successful `tools/call` your server handles now emits a signed atrib record carrying the spec [§1.2](../../atrib-spec.md#12-the-attribution-record) record format, propagates W3C trace context to the response, and submits to your configured log endpoint asynchronously per spec [§5.3.5](../../atrib-spec.md#535-log-submission).

## What the middleware does on every tool call

Per spec [§5.3](../../atrib-spec.md#53-atribmcp-mcp-server-middleware), on every inbound `tools/call`:
...
3. **Constructs the attribution record** with `content_id` derived from `serverUrl` + tool name ([§1.2.2](../../atrib-spec.md#122-content_id-derivation)), `chain_root` from inbound context or genesis ([§1.2.3](../../atrib-spec.md#123-chain_root-for-genesis-records)), `event_type` from the optional `transactionTools` set in your options.
...
- If a tool handler returns `isError: true`, **no record is emitted** per [§5.3.3](../../atrib-spec.md#533-record-construction-and-signing) and no context is written to the response. Errors do not contribute to attribution chains.
...
| `serverUrl`        | `string`         | recommended             | Canonical URL for `content_id` derivation per [§1.2.2](../../atrib-spec.md#122-content_id-derivation). Required for stdio transports where no host header is available.                                           |
...
`@atrib/log-dev` implements spec [§2.6.1](../../atrib-spec.md#261-submit-entry) wire format conformance exactly; anything that flows through it would also be accepted by a real Tessera log. It uses placeholder Merkle hashes and is **not for production use**. See [`packages/log-dev/README.md`](../log-dev/README.md) for the full warning and the package's purpose.
...
- [§5.8](../../atrib-spec.md#58-degradation-contract) degradation contract; every failure mode caught, never propagated
...
| ------------ | -------------------------------------------------------------------------- |
| [§1.2](../../atrib-spec.md#12-the-attribution-record)         | Attribution record format                                                  |
| [§1.3](../../atrib-spec.md#13-canonical-serialization)         | JCS canonicalization (RFC 8785)                                            |
| [§1.4](../../atrib-spec.md#14-signing-and-verification)         | Ed25519 signing and verification                                           |
| [§1.5](../../atrib-spec.md#15-context-propagation)         | Context propagation via `params._meta`, tracestate, baggage, X-atrib-Chain |
| [§2.6.1](../../atrib-spec.md#261-submit-entry)       | Submission API client (POST a bare signed record)                          |
| [§2.6.2](../../atrib-spec.md#262-inclusion-proof-response)       | Proof bundle response shape                                                |
| [§5.3](../../atrib-spec.md#53-atribmcp-mcp-server-middleware)         | Server-side middleware behavior                                            |
| [§5.3.3](../../atrib-spec.md#533-record-construction-and-signing)       | No emission for `isError: true`                                            |
| [§5.3.5](../../atrib-spec.md#535-log-submission)       | Non-blocking submission queue, proof cache, HTTP proof endpoint             |
| [§5.3.6](../../atrib-spec.md#536-policy-exposure)       | Policy exposure via HTTP endpoint and stdio serverInfo                      |
| [§2.8](../../atrib-spec.md#28-proof-bundle-format)         | Proof bundle text format (C2SP tlog-proof serialization and parsing)        |
| [§5.6.3](../../atrib-spec.md#563-key-storage-requirements)       | Key storage: memory zeroing via `zeroize()` and `destroy()` on AtribServer |
