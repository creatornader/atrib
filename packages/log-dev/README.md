...
2. **End-to-end demos and customer walkthroughs.** The runnable demo at `packages/integration/examples/end-to-end/` uses `@atrib/log-dev` so a viewer can `pnpm dev` and watch real attribution records flow through a fake merchant tool, an agent, and into the log without standing up Tessera first. The visible behavior is faithful even though the cryptographic guarantees are not.
