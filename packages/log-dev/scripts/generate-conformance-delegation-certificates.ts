...
async function main(): Promise<void> {
  ...
  writeCase('walk-valid', {
    ...
    expected: {
      ...
      delegation: {
        ...
        cert_valid: true,
        in_window: true,
        time_basis: 'record_timestamp_claim',
        time_trusted: false,
        context_bound: true,
        cert_bound: true,
        scope_check: {
  ...
  writeCase('walk-scope-cost-policy', {
    ...
    spec_section: '1.11',
    description:
      'A certificate whose scope carries the D165 cost_policy sub-field (model_tiers allowlist, max_tokens budget). Signed records carry neither model tier nor token spend, so the record-only walk names both constraints as unverifiable instead of treating the empty mismatch list as completed evaluation. The usage_vectors pin caller-supplied evaluation: within-grant usage is in scope; a tier outside the allowlist plus spend over the budget produce both mismatches. Signals, never invalidation (§6.7.3): the record signature and depth-1 resolution are unaffected.',
    input: {
  ...
  writeCase('revocation-run-key', {
    ...
    expected: {
      revocation_canonical_signing_input_utf8: utf8Decode(canonicalSigningInput(revocation)),
  ...
  writeCase('walk-ambiguous-principals', {
    ...
    input: {
      record: walkValidRecord,
      genesis_record: walkValidRecord,
      certificates: [certB1, ambiguousCert],
    },
  ...
  writeCase('walk-depth-limit', {
    ...
    input: {
      record: depthRecord,
      genesis_record: depthRecord,
      certificates: [parentCert, nestedCert],
    },
