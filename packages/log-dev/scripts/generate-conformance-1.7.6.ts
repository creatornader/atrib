...
async function main(): Promise<void> {
  ...
  const r5 = {
    ...r5Pristine,
    signers: [r5Pristine.signers![0]!, { ...r5Pristine.signers![1]!, signature: tamperedSig }],
  } as AtribRecord
  ...
  // ── Case 6: creator-signer-missing ─────────────────────────────────
  // Two valid counterparty signatures are present, so cross_attestation
  // itself is not missing. The record is still not valid because none of
  // the signer entries matches the top-level creator_key.
  const agentPub = base64urlEncode(await getPublicKey(AGENT_SEED))
  const skeleton: AtribRecord = {
    spec_version: 'atrib/1.0',
    content_id: 'sha256:' + '06'.repeat(32),
    creator_key: agentPub,
    chain_root: genesisChainRoot(ALICE_CONTEXT),
    event_type: 'https://atrib.dev/v1/types/transaction',
    context_id: ALICE_CONTEXT,
  ...
  const canonicalBytes = canonicalCrossAttestationInput(skeleton)
  const counterpartyPub = base64urlEncode(await getPublicKey(COUNTERPARTY_SEED))
  const facilitatorPub = base64urlEncode(await getPublicKey(FACILITATOR_SEED))
  const r6 = {
    ...skeleton,
    signers: [
      {
        creator_key: counterpartyPub,
        signature: base64urlEncode(await ed.signAsync(canonicalBytes, COUNTERPARTY_SEED)),
      },
      {
        creator_key: facilitatorPub,
        signature: base64urlEncode(await ed.signAsync(canonicalBytes, FACILITATOR_SEED)),
      },
    ],
  } as AtribRecord
  writeCase('creator-signer-missing', {
    name: 'creator-signer-missing',
    spec_section: '1.7.6',
    description:
      "A transaction record whose signers[] contains two valid counterparty signatures but no signer entry for the top-level creator_key. Verifier MUST surface cross_attestation as satisfied (signers_valid: 2, missing: false) while rejecting the record's base signature path because unrelated signers do not validate the creator.",
    input: {
      record: r6,
      signer_seeds_hex: {
        counterparty: hex(COUNTERPARTY_SEED),
