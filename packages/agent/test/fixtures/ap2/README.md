...
- autonomous mode: an L2 user mandate that discloses open checkout and
  payment mandates delegated to an agent key, plus L3 checkout/payment
  mandates signed by that agent key.

The VI fixtures use static test keys and compact SD-JWT strings. The signed
autonomous success fixture and negative matrix are generated by
`generate-vi-fixtures.mjs` so the corpus is deterministic and locally
replayable. The AP2 receipt `reference` values bind to the disclosed
...
- `vi_immediate_evidence.json`: signed VI immediate-mode evidence plus AP2
  ...
  mandates.
- `vi_autonomous_success_evidence.json`: signed VI autonomous-mode success
  evidence with an L1 issuer credential, L2 open checkout/payment mandates,
  L3 closed checkout/payment mandates, successful checkout and payment
  receipts, final hash bindings, and seven passing typed constraints.
- `vi_autonomous_negative_matrix.json`: named negative cases applied to
  `vi_autonomous_success_evidence.json`. The matrix covers tampered L2
  signature, tampered L3 signature, disclosure digest mismatch, `sd_hash`
  mismatch, wrong L3 agent key, wrong checkout hash, wrong transaction id,
  wrong receipt reference, expired credential, and missing issuer key.
...
- `vi_autonomous_constraints_decoded.json`: synthetic decoded open-mandate
  ...
  execution windows.
- `generate-vi-fixtures.mjs`: deterministic local generator for the signed
  autonomous success fixture and negative matrix. It derives test-only P-256
  keys from fixed fixture seeds and must not be used outside fixture
  generation.
