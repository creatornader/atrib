...
Public files describe this infrastructure in role terms only (orchestrator,
relay, executor harness, loop layer): no operator-internal product names,
incident figures, or subscription details. `pnpm doc-sync` enforces the
vocabulary locally through a private-wordlist check whose pattern file lives
