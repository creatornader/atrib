...
atrib/
  ...
  DECISIONS.md                 # Architectural decision log (D001-D084; D070 + D073 are placeholder ADRs for the Record Body Archive Layer per §2.12 and the `handoff` event_type byte respectively; D071 codifies spec writing conventions; D072 governs orphan handling; D074 + D075 capture the git-trailer integration and compose-not-override hook config patterns; D076 introduces the opt-in long-lived atrib-emit daemon; D077 codifies pass^k as the primary Track B reporting metric, k=3 default, promoted from P019 on 2026-05-10; D081 introduces emitInProcess for hook-class producers (signing in-process, byte-identical to MCP-signed records); D082 supersedes D081's integration shape by shipping `atrib-emit-cli` from `@atrib/emit` and having the hook helper spawn the binary, so the hook source directory never becomes an npm workspace; D083 extends D078 with a harness session-id discovery registry in `@atrib/mcp` so cognitive-primitive MCP servers spawned by Claude Code, and future harnesses, derive `context_id` from the parent's session_id env var without operator-side config (v2 2026-05-23 adds optional file-fallback to the registry for startup-spawn harnesses like Claude Code whose MCP children pre-date the per-session env; SessionStart hook writes `~/.claude/state/active-session-id-<ppid>` and `resolveEnvContextId` reads it when env is unset); D084 ships per-event instrumentation jsonl files under `~/.atrib/state/` for read primitives (Surface 6, via `logReadPrimitiveCall` in `@atrib/mcp@0.10.0`), SessionStart (Surface 7), cli-spawn transport (Surface 8), plus a unified analyzer (Surface 9, `atrib-internal/tools/claude-hooks/analyze-substrate.mjs`) that joins all four jsonl pillars + the signed-record mirror so the loop-closure question can be answered against data instead of inferred from one source; `@atrib/recall` compact-mode response now always carries `record_hash` so callers can chain other primitives from any result). A "Pending decisions" section at the end (forward-looking pattern) tracks a set of forward-looking decisions awaiting action (P002, P004, P005, P008, P009, P010, P012-P018, P020, P021). P012-P017 cover: P012 new §9 pattern for multi-agent orchestrator pre-delegation verification, P013 new §9 pattern for hosted-runtime adapter, P014 new §9 pattern for sandboxed-execution composition, P015 §1.4 normative MUST signing-key-outside-sandbox (SECURITY), P016 §0 positioning extension EveryDev mapping, P017 §8.7 Layer 11 environment-isolation boundary-trust. P018, P020, P021 cover remaining eval framework adoption: P018 Inspect AI as Track B harness baseline, P020 conformance corpus extension to Wycheproof-equivalent coverage, P021 publishable behavior-impact paired benchmark suite.
