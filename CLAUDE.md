...
| HSM/KMS profile added or modified ([D037](DECISIONS.md#d037-hsmkms-operator-profile)) | Update `packages/agent/src/keystore-<profile>.ts` (or wrapper-side equivalent); document operator runbook at `docs/operator/hsm-<profile>.md` (operator-internal staging first); the `keystore: 'callback'` middleware contract is normative and MUST NOT change without a coordinated wrapper update + ADR amendment. |
