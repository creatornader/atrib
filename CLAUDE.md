...
atrib/
  ...
  packages/
    ...
    runtime-log/               # @atrib/runtime-log: Runtime-log proof helpers. Publish target until first manual npm publication. Builds and verifies log_window_manifest objects for host-owned agent run windows per D121. Does not sign records, store raw logs, or replace host runtime state.
...
- **Nine SDK and integration packages in the npm publish set** (`@atrib/mcp`, `@atrib/agent`, `@atrib/verify`, `@atrib/cli`, `@atrib/mcp-wrap`, `@atrib/directory`, `@atrib/openinference`, `@atrib/memory-tool`, `@atrib/runtime-log`; `@atrib/runtime-log` remains pending first manual npm publication)
