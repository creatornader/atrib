...
A subset of these triggers are mechanically enforced by `scripts/check-doc-sync.mjs` (run via `pnpm doc-sync`, also wired into CI). It catches number-word drift between canonical sources of truth and target docs (edge type count, node type count, dashboard view count, public package count, workspace package list completeness). When adding a new "<word> <thing>" claim that should stay in sync with an enumeration somewhere, extend the script with a new check rather than relying on discipline alone.
