...
The walkthrough is hosted as a separate stable artifact at `https://explore.atrib.dev/walkthrough`. It is a scripted scenario and deliberately does not replace `/replay`, the live recent-action replay: one is staged so it can teach, the other stakes its claim on nothing being staged. `/yc-demo`, `/yc-demo/` and `/yc-demo.html` redirect here, as do `/demo` and `/demo/` to `/replay`.
...
| `/replay`                 | (none)                               | log `/v1/recent` + graph `/v1/graph/<context_id>` when available                                         | ✅ live recent-action replay graph paired with a concise agent-session timeline                                                                                                                                                      |
...
| Live recent-action replay (`/replay`)     | ✅ Live    | Sigma + dagre                     | Selects the busiest recent session, renders a concise agent timeline, and animates nodes and edges into view. Falls back to a tested log-derived replay graph if graph-node has no usable graph for the session.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
