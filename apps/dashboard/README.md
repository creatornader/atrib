...
The explorer's Open Graph card, touch icon, and favicon live under `apps/dashboard/static/`. The PNG assets are byte-identical copies of files generated in `atrib-web` by `pnpm assets`; `favicon.ico` is copied from `atrib-web/app/favicon.ico`. Keep the hashes aligned so `atrib.dev` and `explore.atrib.dev` do not present different brands in feeds or browser chrome.
