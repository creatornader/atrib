...
> 1038 tests across all packages and services. Public packages are not yet published to npm. Three production services: [`services/log-node/`](services/log-node/) (`https://log.atrib.dev/v1`), [`services/graph-node/`](services/graph-node/) (`https://graph.atrib.dev/v1`), [`services/directory-node/`](services/directory-node/) (`https://directory.atrib.dev/v6`). A reproducible end-to-end verifier with 13 gate assertions across 8 named categories (tree integrity, format conformance, checkpoint signature, pubkey-publication agreement, signer scope, attribution, record signature replay, chain integrity) ships at `services/log-node/scripts/verify-loop.mjs` and runs daily in CI against the deployed log. The public block explorer at [`https://explore.atrib.dev/`](https://explore.atrib.dev/) composes all three services into 5 views (overview, identity, session, action, anchoring). A fourth standalone MCP service, [`services/atrib-emit/`](services/atrib-emit/), exposes one tool the agent invokes explicitly to sign cognitive events the wrapper doesn't auto-capture (observations, annotations, revisions); records are byte-identical to wrapper-signed records and inherit the wrapper's session via the local mirror.
