...
| --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Action control              | A host can sign an allow, block, escalate, or approval decision before a selected action runs. The outcome cites the decision, and the protected MCP adapter requires a one-time server-side permit plus a non-revoked credential before the upstream side effect when the host supplies a live revocation view.                                               | [`@atrib/action-gate`](packages/action-gate/README.md), policy engines, approvals                                                                                                                                                                            |
...
| Shared current state        | The `recall` state view projects verified revision lineages into current heads under a caller-supplied signer and context policy. Forks stay visible as unresolved conflicts instead of collapsing to an arbitrary winner. The open explorer shows the corresponding public revision commitments while keeping its weaker browser verification basis explicit. | Multi-agent workspaces, incident rooms, support teams, fleet control surfaces                                                                                                                                                                                |
...
| A host-owned run log, event stream, session history, checkpoint log, fork log, or compaction log | [`@atrib/runtime-log`](packages/runtime-log/README.md), including [`@atrib/runtime-log/buzz`](packages/runtime-log/README.md#buzz-observer-source)                                                                                                               | A `log_window_manifest` that commits to the bounded run window, roots, projections, receipts, and redaction policy without publishing raw log bodies by default.   |
...
[Examples and proofs](#examples-and-proofs) below.
...
[`packages/integration/examples/`](packages/integration/examples/) (proof
packets under [`proof-packets/`](proof-packets/)). Start with the row matching
your stack.
...
| A2A handoff evidence                    | [`packages/integration/examples/a2a-handoff/`](packages/integration/examples/a2a-handoff/) signs an A2A v1 `AgentCard`, carries an atrib evidence packet in a data-valued A2A `Part`, verifies the packet, and signs the receiver follow-up through `informed_by`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| ---------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`@atrib/sdk`](packages/sdk/README.md)                     | Consolidated client SDK: `attest()`/`recall()` cognitive verbs plus `action()` for paired request/outcome evidence around an application-owned execution boundary. Daemon-first with in-process fallback, and the full [§1](atrib-spec.md#1-attribution-record-format) record layer re-exported. Byte-identical Python sibling at [`python/`](python/README.md).                                                        |
...
| [`@atrib/action-gate`](packages/action-gate/README.md)     | Host-owned action gate. Signs policy decisions and outcomes before selected high-impact actions run, with a protected MCP executor that rejects missing, mismatched, expired, and replayed permits before the upstream side effect.                                                                                                                                                                                     |
| [`@atrib/verify`](packages/verify/README.md)               | Merchant and substrate verification. Re-runs the calculation locally, checks AP2 / VI, authorization, Nostr, and Buzz owner-attestation evidence, verifies Pattern 3 handoff claims, and applies pinned-key checkpoint witness thresholds.                                                                                                                                                                              |
| [`@atrib/cli`](packages/cli/README.md)                     | CLI: one-command named principal, workspace, agent, and run setup; Keychain key management; identity claims; key rotation and revocation.                                                                                                                                                                                                                                                                               |
...
| [`@atrib/runtime-log`](packages/runtime-log/README.md)     | Runtime-log proof helpers. Builds and verifies bounded run-window manifests plus expected-surface coverage manifests that distinguish captured, skipped, degraded, and omitted actions. The [`buzz`](packages/runtime-log/README.md#buzz-observer-source) subpath verifies live or archived NIP-AO observer captures and commits complete decrypted objects without publishing them.                                    |
...
| [`@atrib/operating-graph`](apps/operating-graph/README.md)         | Open-source reference client for named, bounded workspace views, body-aware search, live decisions and outcomes, handoffs, explicit conflicts, and signed application resolutions.                                                                      |

The twenty designed-public packages split fourteen current and six deprecated;
the public-package tables above are the canonical list. Private application and
test packages do not change that count. The [`python/`](python/README.md)
directory ships the `atrib` Python SDK (published on PyPI), the first
non-TypeScript implementation of the
[§1](atrib-spec.md#1-attribution-record-format) record layer, held
byte-identical to the TypeScript implementation by the shared conformance
corpora and a cross-implementation determinism harness.

## Open-source operating graph

[`apps/operating-graph/`](apps/operating-graph/) is a complete inspectable
...
through `informed_by` and its private body names the selected head. Local,
container, fresh-machine, and application-conformance instructions ship with
the client.

Its revision stream carries exact event IDs and reports missed local projection
revisions as gaps that require a bounded-view reload. Full signed-body and
opening disclosure is disabled by default. Operators can enable a separate
...
- [`atrib-log-pp-cli`](https://github.com/creatornader/atrib-log-pp-cli): a [Printing-Press](https://github.com/mvanhorn/cli-printing-press)-generated Go CLI for the public log API (signed checkpoint, recent entries, lookups by hash / context / creator, Merkle tiles, local SQLite mirror). Complementary to the `attest`/`recall` MCP verbs: agents use the verbs, humans and scripts use the CLI.
