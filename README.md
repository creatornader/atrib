...
| [`atribd`](services/atribd/README.md)                              | Public stateless-native local daemon (publish target, first publish pending): serves all seven primitive packages as fifteen alias-mounted tools over stateless Streamable HTTP, direct stdio, or a stdio-to-HTTP proxy shim, with per-context write serialization. The recommended local topology per [D147](DECISIONS.md#d147-atribd-is-the-public-stateless-native-local-daemon-for-the-primitive-runtime). |
| [`@atrib/primitives-runtime`](services/atrib-primitives/README.md) | Private local MCP runtime: exposes all seven primitive packages through one stdio or Streamable HTTP MCP server for dogfood harnesses that would otherwise spawn one process per primitive. Superseded as the recommended topology by `atribd`.                                                                                                                              |
