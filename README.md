...
**Spec-defined but not implemented:** Witnessing (§2.9, D032; first implementation deferred until a non-operator verifier exists). Key rotation and revocation (§1.9, D033; implementation in Phase 3 of the gap-closure plan). Public-key directory (§6, D034; same Phase 3, AKD-based, unblinded mode for atrib and a VRF-blinded mode available for downstream consumers requiring privacy-preserving lookup).
