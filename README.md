...
| [`@atrib/attest`](services/atrib-attest/README.md)                 | The write verb (first-publish pending): one `attest` tool signs observations, annotations (`ref.kind: "annotates"`), and revisions (`ref.kind: "revises"`), with the legacy `emit` / `atrib-annotate` / `atrib-revise` tool names mounted as permanent aliases over the same handler. |
| [`@atrib/emit`](services/atrib-emit/README.md)                     | Legacy home of the `emit` write primitive; re-export shim over `@atrib/attest`, binaries forward. Records are byte-identical. |
