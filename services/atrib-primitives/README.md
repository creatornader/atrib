...
The protocol tests list all seventeen tools over stdio, route a recall call through the combined server, repeat the path through atribd's stateless Streamable HTTP host, and check the stdio proxy path. A process test pins the client to MCP 2026-07-28 across the private runtime and all eight standalone primitive binaries. The suite also checks starting health, explicit-context profiles, every primitive surface, deterministic non-mutating behavioral probes, recall content-index support, and bounded timeouts for hung child primitives. `pnpm doc-sync` runs `scripts/check-primitives-runtime-update.mjs` so LaunchAgent selection, primitive surfaces, behavioral probes, and the direct recall payload contract stay covered.
