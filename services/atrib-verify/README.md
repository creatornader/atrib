...
MCP server exposing the `atrib-verify` primitive. It verifies counterparty handoff evidence before a receiving agent signs follow-up work that cites those records through `informed_by`.
...
- [D062](../../DECISIONS.md#d062-local-mirror-sidecar--two-tier-private-local--public-canonical-persistence) local mirror envelopes: `{ record, proof?, _local?: { content?, args?, result? } }`
...
Add to an MCP host config:

```json
{
  "atrib-verify": {
    "command": "node",
    "args": ["/path/to/atrib-verify/dist/main.js"]
  }
}
```

Or run as a one-off subprocess:
