...
import { traceBackward, traceForward, type TraceVisited } from './trace-walk.js'
...
export function summarizeSidecar(
  loadedRecord: { local?: SidecarPayload } | undefined,
...
export function runTraceWalk(
  ...
  return buildPayload(result, byHash, compact, includeContent) as Record<string, unknown>
}

/** Register the legacy `trace` + `trace_forward` tools on a server. */
export function registerTraceTools(mcp: McpServer): void {
  mcp.registerTool(
    'trace',
    {
