...
describe('end-to-end proof verification', () => {
  it('independently verifies that a submitted record is included in the log', async () => {
    ...
    expect(Buffer.from(computedLeafHash).toString('hex')).toBe(
      returnedLeafHash.toString('hex'),
    );
    ...
    const proofHashes: Uint8Array[] = proof.inclusion_proof.map((h) =>
      new Uint8Array(Buffer.from(h, 'base64')),
    );
