...
// The CLI serves two bin names: `atrib-emit-cli` (legacy, forwarded by the
// @atrib/emit shim) and `atrib-attest-cli` (the write-verb home's own bin).
// The identity is derived from the invoked entrypoint's basename so one
// implementation serves both. Ambiguous invocations (node dist/cli.js) keep
// the legacy label: operator hooks that spawn the dist path directly must
// keep stamping 'atrib-emit-cli' into `_local.producer` (persisted-label
// rule L1: historical labels stay stable; only attest-named entrypoints
// stamp the new family).
type CliBinName = 'atrib-emit-cli' | 'atrib-attest-cli'

function detectBinName(argv1: string | undefined): CliBinName {
  if (typeof argv1 === 'string' && argv1.length > 0) {
    const base = argv1.split('/').pop() ?? ''
    if (base.includes('attest')) return 'atrib-attest-cli'
  }
  return 'atrib-emit-cli'
}

const BIN_NAME: CliBinName = detectBinName(process.argv[1])
  ...
  return {
    ...
    envelope_schema: {
      ...
      optional: {
        ...
        producer:
          'Producer label routed to mirror sidecar `_local.producer`. Defaults to the invoked bin name ("atrib-emit-cli" or "atrib-attest-cli"); hook helpers override with finer attribution (e.g. "claude-hooks-builtin-2b").',
        local_substrate:
...
interface RawEnvelope {
   ...
   * When omitted, the CLI labels records with the invoked bin name
   * (`'atrib-emit-cli'` or `'atrib-attest-cli'`). Hook-class
...
function buildLocalSubstrateCommitOption(
  ...
  if (!entryId || !sourcePath || !receiptJoinField || !joinBackTarget) {
    writeStderrLine(
      `${BIN_NAME}: local_substrate commit skipped; wal entry_id, source_path, receipt_join_field, and join_back_target are required`,
    )
    return undefined
  }
  ...
  if (!endpoint) {
    writeStderrLine(
      `${BIN_NAME}: local_substrate commit skipped; ATRIB_LOCAL_SUBSTRATE_ENDPOINT is unset`,
    )
    return undefined
  }
...
export async function main(argv: readonly string[]): Promise<number> {
  ...
  try {
    ...
    writeStderrLine(
      `${BIN_NAME}: stdin read error: ${e instanceof Error ? e.message : String(e)}`,
    )
  ...
  try {
    ...
    writeStderrLine(
      `${BIN_NAME}: stdin parse error: ${e instanceof Error ? e.message : String(e)}`,
    )
  ...
  try {
    ...
    if (!result.signed) {
      writeStderrLine(
        `${BIN_NAME}: refused refusals=${result.refusals.length} elapsed=${elapsed}ms`,
      )
      return 3
    }
    writeStderrLine(
      `${BIN_NAME}: ok record_hash=${String(result.record_hash).slice(0, 24)}… log_index=${result.log_index ?? 'null'} ` +
